...
exports.registerUser = catchAsyncErrors(async (req, res) => {
  const { email,token,name,phoneNumber } = req.body;
  ...
  if (user) {
    ...
    try {
      const newUser = await userdb.create({ email,token,name,phoneNumber });
...
exports.getusersurl =catchAsyncErrors(async(req,res)=>{
  const {linkedinurl,githuburl}=req.body;
  try {
    const data = await userdb.findByIdAndUpdate(req.user._id,{linkedinurl,githuburl},{new:true})
    res.status(200).send({update:true,data});
  } catch (error) {
    res.status(500).send({update:true,data});   
  } 
})
exports.getusersupiid =catchAsyncErrors(async(req,res)=>{
  const {upiid}=req.body;
  try {
    const data = await userdb.findByIdAndUpdate(req.user._id,{upiid},{new:true})
    res.status(200).send({update:true,data});
  } catch (error) {
    res.status(500).send({update:true,data});
  } 
