...
exports.handleEmailNotificationProcess = async (email,res) => {
  const client = await clientdb.findOne({ email });
  // Calculate the next valid request time dynamically
  // let nextValidRequestTime = clientdb  ? new Date() : null;
...
// 30 minutes expiry
  ...
  (async function () {
    const { data, error } = await resend.emails.send({
      ...
      to: [client.email],
      subject: "Your One-Time Password",
      html: `<p>Complete Your Profile , Your password for login</p><br/><h1><strong> ${client?.adminpassword}</strong></h1><p>Thank you for using Krunch</p>`,
    });
  ...
  // Update the user record with the new otp and otpExpiry
await clientdb.updateOne({ email }, { isapproved:true });
};
